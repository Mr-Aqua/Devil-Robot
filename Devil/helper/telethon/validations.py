from Devil.helper.telethon import IMMUNE_USERS, telethn
...
async def user_is_admin(user_id: int, message):
    ...
    async for user in telethn.iter_participants(
        ...
        if user_id == user.id or user_id in ENFORCER:
            status = True
            break
    return status
...
async def Devil_is_admin(chat_id: int):
    status = False
    Devil = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Devil.id == user.id:
