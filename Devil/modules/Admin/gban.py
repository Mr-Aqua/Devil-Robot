...
import Devil.Database.global_bans_sql as sql
...
from Devil.Functions.misc import send_to_list
...
def gban(update: Update, context: CallbackContext):
    ...
    if int(user_id) in ENFORCER:
        message.reply_text(
            "OOOH someone's trying to gban a enforcer! *grabs popcorn*",
        )
        return
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot, INSPECTOR + ENFORCER, log_message, html=True)
        ...
        try:
            ...
            if excp.message in GBAN_ERRORS:
                ...
                if EVENT_LOGS:
                    ...
                    send_to_list(
                        bot, INSPECTOR + ENFORCER, f"Could not gban due to: {excp.message}",
                    )
...
def ungban(update: Update, context: CallbackContext):
    ...
    if EVENT_LOGS:
        ...
        send_to_list(bot, INSPECTOR + ENFORCER, "un-gban complete!")
...
def __user_info__(user_id):
    ...
    if user_id == dispatcher.bot.id:
        return ""
    if int(user_id) in INSPECTOR + ENFORCER:
        return ""
    if is_gbanned:
