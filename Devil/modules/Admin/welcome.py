...
import Devil
import Devil.Database.welcome_sql as sql
import Devil.Database.global_bans_sql as gban_sql
from Devil import (
...
def new_member(update: Update, context: CallbackContext):
    ...
    for new_mem in new_members:
        ...
        if should_welc:
            ...
            # Welcome Support
            elif new_mem.id in ENFORCER:
                update.effective_message.reply_text(
                    "Huh! Someone with a enforcer level just joined!",
                    reply_to_message_id=reply,
                )
                welcome_log = (
                    f"{html.escape(chat.title)}\n"
                    f"#USER_JOINED\n"
                    f"Bot enforcer just joined the group"
                )
                continue
...
def welcome(update: Update, context: CallbackContext):
    ...
    if not args or args[0].lower() == "noformat":
        ...
        if args[0].lower() in ("on", "yes"):
            ...
            update.effective_message.reply_text(
                "I'll go loaf around and not welcome anDevil then.",
            )
